...
GROUP_IDENTIFIER = os.environ["TG_GROUP"]  # @username or numeric chat id
...
CURRENCY_BLOCK_RE = re.compile(
    ...
    r"\s*D1\s*:\s*([+-]?\d+)\s*\|?\s*H4\s*:\s*([+-]?\d+)\s*\|?\s*H1\s*:\s*([+-]?\d+)",
    re.IGNORECASE,
)
...
def parse_scorecard(text: str) -> dict:
    ...
    for currency, d1, h4, h1 in matches:
        scores[currency.upper()] = {"D1": int(d1), "H4": int(h4), "H1": int(h1)}
    return scores
