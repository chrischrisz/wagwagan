...
def classify(score: int) -> str:
    """
    STRONG = positive extreme (4 to 6)
    WEAK   = negative extreme (-4 to -6)
    NEUTRAL = anything in between (-3 to +3, either sign)
    """
    if score >= 4:
        return "STRONG"
    elif score <= -4:
...
def build_shortlist(currency_extremes: dict) -> list:
    """
    Builds the filtered BUY/SELL shortlist (|gap| >= 5).
    Bias applies whenever base and quote fall into different categories
    (STRONG/WEAK/NEUTRAL) - same category on both sides = range, no bias.
    Direction follows the sign of the gap: positive = BUY, negative = SELL.
    """
    shortlist = []
    for pair in PAIRS:
        ...
        score_label = f"{base_class} vs {quote_class}"
        shortlist.append({"pair": pair, "gap": gap, "score": score_label, "bias": bias})
    return shortlist
...
def format_telegram_message(shortlist: list) -> str:
    ...
    for row in shortlist:
        ...
        lines.append(f"{row['pair']}: {row['bias']} (gap {sign}{row['gap']}, {row['score']})")
    return "\n".join(lines)
...
def log_shortlist(ws_bias_log, shortlist: list):
    ...
    rows = [[timestamp, row["pair"], row["gap"], row["score"], row["bias"]] for row in shortlist]
    if rows:
...
def main():
    ...
    ws_bias_log = get_or_create_tab(
        sh, SHEET_TAB_BIAS_LOG, ["Timestamp", "Pair", "Gap", "Score", "Bias"]
    )
